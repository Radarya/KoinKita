...
pattern = re.compile(
    r"<button\s+onClick=\{\(\) => \{\s*playClick\(\);\s*setShowTopicSelection\(true\);\s*\}\}\s+className=\"group relative[^\"]+\"\s*>[\s\S]*?<Share2 className=\"w-5 h-5 text-purple-500\" />\s*\{language === 'id' \? 'Bagikan' : 'Share'\}\s*</button>\s*</div>", 
    re.DOTALL
)
